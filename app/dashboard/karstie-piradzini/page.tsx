    ...
    <div className="mx-auto max-w-4xl px-6 py-10">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-extrabold text-gray-900">
            <Flame size={22} className="text-orange-500" /> Karstie pīradziņi
          </h1>
          <p className="mt-0.5 text-sm text-gray-500">
            {active.length} aktīvi · {past.length} iepriekšējie
          </p>
        </div>
        <Link href="/dashboard/karstie-piradzini/jauns"
          className="btn-primary flex items-center gap-2">
          <Plus size={16} /> Jauns pīradziņš
        </Link>
      </div>
      ...
      {drops.length === 0 ? (
        <div className="rounded-2xl border-2 border-dashed border-orange-200 bg-orange-50/30 p-16 text-center">
          ...
          <p className="mt-3 font-semibold text-gray-900">Vēl nav neviena pīradziņa</p>
          ...
          return (
            <div key={drop.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <Link href={`/karstie-piradzini/${drop.id}`} target="_blank"
                ...
                {isActive && (
                  <>
                    <Link href={`/dashboard/karstie-piradzini/${drop.id}/edit`}
                      className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-700 transition"
                      title="Rediģēt">
                      <Flame size={14} />
                    </Link>
